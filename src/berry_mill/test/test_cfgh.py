...
class TestCollectionAutodict:
	...
	def test_autodict_get_item_with_nonexistant_key(self, autodict_instance: Autodict):
		...
		subautodict = autodict_instance[key]
	...
	def test_autodict_get_item_with_nondefined_value(self, autodict_instance: Autodict):
		# set only key
		sub_dict = autodict_instance["key"]
	...
	def test_autodict_set_item_with_defined_dict_value(self, autodict_instance_with_value: Autodict):
		try:
			# set  dict type and expect exception
			autodict_instance_with_value["key1"] = {"test_key2":"test_value2"}
		except Exception as e:
			assert str(e) == "configuration type error: unable directly set to a hash, data container needs an update instead"
...
class TestCollectionConfigHandler:
	...
	def test_add_config_that_not_exists(self, capsys, config_handler: ConfigHandler):
		# Set non existent path
		non_existent_config_path = "/path/to/non/existent/config.cfg"
		# add non existent path
		config_handler.add_config(non_existent_config_path)
		# capture standard output
		output_msg = capsys.readouterr()
	...
	def invalid_config(self):
		...
		invalid_config_data = 'key1: value1\nkey2;value2'
	...
	def test_parse_valid_config(self, config_handler, valid_config):
	...
	def test_parse_invalid_config(self, config_handler, invalid_config, capsys):
		try:
	...
	def test_load_with_wrong_config_path(self, config_handler, capsys):
		try:		
			# Mock cfg paths
			config_handler._cfg = ['/path/to/config1','/path/to/config2']
			# Load configs
			config_handler.load()
			# Capture the error message printed on stdout
			captured = capsys.readouterr()	
