...
class KiwiBuilder(KiwiParent):
    ...
    def _write_repokeys_box(self, repos: Dict[str, Dict[str, str]]) -> bool:
        ...
        for reponame in repos:
            ...
            try:
                ...
                log.warning(f"Failure while trying to copying the keyfile at {str(parsed_url.path)}", exc_info=exc)
                return False
        return True
    ...
    def process(self) -> None:
        ...
        if self._params.get("local"):
            command = ["kiwi-ng"] + kiwi_options + ["system", "build", "--description", "."] + ["--target-dir", target_dir]
            try:
